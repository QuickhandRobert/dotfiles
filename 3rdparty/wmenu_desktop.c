...
#define TERM   "foot"
#define WM_BG  "#000000" // normal background color
#define WM_FG  "#DADADA" // normal text color
#define WM_SBG "#006773" // selection background
#define WM_SFG "#FBFBFB" // selection text color
#define WM_PT  "Apps:" // prompt to the left of the input field
#define WM_PBG "#000000" // prompt background color
#define WM_PFG "#FBFBFB" // prompt text color
#define DEFAULT_PATH1 "/usr/share/applications"
#define DEFAULT_PATH2 "/home/farbod/.local/share/applications"
...
char* constr_comm (DFlist *df_list){
    ...
    char *command;
    asprintf(&command, "echo \"%s\" | wmenu -N \"%s\" -n \"%s\" -S \"%s\" -s \"%s\" -p \"%s\" -M \"%s\" -m \"%s\"",
...
int main (int argc, char **argv) {
    ...
    DFlist df_list = {0};
    char *path1 = DEFAULT_PATH1;
    char *path2 = DEFAULT_PATH2;
    char *paths[2];
    paths[0] = path1;
    paths[1] = path2;
    if (parse_paths(paths, 2, &df_list)){
        return 0;
    };
